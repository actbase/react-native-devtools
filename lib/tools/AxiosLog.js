...
var colorForStatus = function (status) {
  if (typeof status === 'undefined') return;else if (status >= 200 && status < 400) return '#228822';else if (status >= 400 && status < 500) return '#c82';else if (status >= 500) return '#C00';
  return;
};
...
var AxoisLog = function () {
  ...
  var _d = useASStoredState('axios_log_fontSize', 14),
      fontSize = _d[0],
      setFontSize = _d[1];
  ...
  var innerTools = function (extra) {
    ...
    }, React.createElement(ClearIconView, {
      color: 'red'
    })), extra);
  };
